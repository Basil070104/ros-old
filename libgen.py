...
class MPdict:
    ...
    def __init__(self, args, motion, dtype) -> None:
        self.args = args
        self.motion = motion
        self.dtype = dtype
        self.init()
    ...
    def init(self):
        ...
        self.cfg_ID    = f"{self.total}-{self.H}-{self.segments}-{self.branch}-{self.range[1]}-{self.args.dist}-{self.speed}-{self.args.wheel}"
        ...
        self.PATH = f"./lib/{self.cfg_ID}.lib"
        if OS.path.exists(self.PATH):
            self.ctrls = torch.load(self.PATH)
            log(option="empc", msg=f"Load trajectory dictionary\t(FILE: {self.PATH})", color=True)
            # if self.speed != 1.0: self.ctrls = torch.concat((self.ctrls, torch.load(self.PATH.replace(str(self.speed), "1.0"))))
            return self.ctrls
        
        # ---

        log(option="empc", msg=f"Generate trajectory dictionary\t(FILE: {self.PATH})", color=True)
        ...
        log(option="empc", msg=f"Save trajectory dictionary\t(FILE: {self.PATH})", color=True)
